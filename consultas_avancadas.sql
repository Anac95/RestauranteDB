...
INSERT INTO Funcionarios (nome, cpf, data_nascimento, endereço, telefone, email, cargo, salario, data_admissao) values 
...
case when salario > 3000 then 'Acima da média' else 'Abaixo da média' end as media_salario from funcionarios;
...
id_produto int auto_increment key,
nome varchar(255) not null,
descricao text,
preco decimal(10, 2),
...
insert into produto (nome, descricao, preco, categoria) value
...
select nome, preco from produto where categoria = 'prato principal' order by nome limit 2 offset 6;
...
select nome, preco, ROW_NUMBER() OVER (ORDER BY PRECO DESC) AS Ranking from produto ORDER BY PRECO DESC LIMIT 5;

SELECT categoria,
AVG(preco) AS media_preco from produto group by categoria; 
...
select * from pedidos;
insert into pedidos (id_cliente, id_funcionarios, id_produto, quantidade, data_pedido, status_pedido) values
...
select id_pedido, coalesce(status_pedido, 'Cancelado') as status from pedidos;
...
id_info int auto_increment key,
id_produto int,
ingredientes text,
...
from info_produtos group by fornecedor;

select fornecedor,
count(*) as quantidade_produtos from info_produtos group by fornecedor having count(*) > 1;
...
select ped.id_pedido, ped.quantidade, ped.data_pedido, cli.nome, cli.email
from pedidos ped
join cliente cli on ped.id_cliente = cli.id_cliente;

select ped.id_pedido, ped.quantidade, ped.data_pedido, cli.nome as nome_cliente, cli.email, func.nome as nome_funcionario
from pedidos ped 
join cliente cli on ped.id_cliente = cli.id_cliente
join funcionarios func on ped.id_funcionarios = func.id_funcionarios;

select ped.id_pedido, ped.quantidade, ped.data_pedido, cli.nome as nome_cliente, cli.email, func.nome as nome_funcionario, prod.nome as nome_produto, prod.preco
from pedidos ped 
join cliente cli on ped.id_cliente = cli.id_cliente
join funcionarios func on ped.id_funcionarios = func.id_funcionarios
join produto prod on ped.id_pedido = prod.id_produto;

select cli.nome from pedidos ped 
join cliente cli on ped.id_cliente = cli.id_cliente
where ped.status_pedido = 'Pendente'
order by ped.id_pedido desc;

select cli.nome from cliente cli left join pedidos ped on cli.id_cliente = ped.id_cliente where ped.id_cliente is null;

select cli.nome, count(ped.id_pedido) as total_pedidos
from cliente cli 
left join pedidos ped on cli.id_cliente = ped.id_cliente group by cli.id_cliente;

select ped.id_pedido, sum(ped.quantidade * prod.preco) as total_preço 
from pedidos ped 
join produto prod on ped.id_produto = prod.id_produto group by ped.id_pedido;
...
(p.quantidade * p.preco) as total_pedido
from
pedidos p
join 
    cliente c on p.id_cliente = c.id_cliente
join
	funcionarios f on p.id_funcionarios = f.id_funcionarios
join 
    produto pr on p.id_produto = pr.id_produto;

select pedido_id, cliente_nome, total_pedido from resumo_pedido;
...
select pedido_id, cliente_nome, total from resumo_pedido;
...
deterministic
begin
     declare ingredientes_texto text;

select ingredientes
into ingredientes_texto
from info_produtos
...
BEGIN
    ...
    SELECT AVG(total) INTO media_geral
    FROM (
        SELECT id_pedido, SUM(preco * quantidade) as total
        FROM pedidos
        GROUP BY id_pedido
    ) AS totais_pedidos;
    ...
    ELSE
        SET mensagem = 'O total do pedido é igual a média';
    END IF;
